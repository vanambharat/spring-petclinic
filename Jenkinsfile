...
stages {
        ...
        steps {
            ...
            withCredentials([string(credentialsId: 'sonar-token', variable: 'SONAR_TOKEN_VAR')]) {
                withSonarQubeEnv('sonarqube') {
                    sh 'mvn org.sonarsource.scanner.maven:sonar-maven-plugin:3.9.1.2588:sonar -Dsonar.projectKey=petclinic -Dsonar.token=${SONAR_TOKEN_VAR}'
                }
            }
        }
    }
    ...
    stage('Trivy Image Scan') {
        steps {
            ...
            // We need to install Trivy on the Jenkins agent first, then run this.
